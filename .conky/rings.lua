...
function conky_ifs_agg(comm)
	local agg = 0
	local convert_back = false
	local if_list = io.popen("~/.conky/ifs_up.sh")
	for if_ in if_list:lines() do
		local result = conky_parse("${"..comm.." "..if_.."}")
		if string.match(result, "[^0-9.]") ~= nil then
			convert_back = true
	--		print("--")
	--		print(result)
			result = si_to_raw(result)
	--		print( result)
		end
		agg = agg + result
	end
	if_list:close()
	if convert_back then return raw_to_si(agg) end
	return agg
end

function si_to_raw(result)
	local ext = string.gsub(result,"^[0-9.]+", "")
	local number = string.gsub(result,"[^0-9.]+$", "")
--	print("'"..ext.."'")
--	print(number)
	if ext:lower() == "b  " then return number*1 end
	if ext:lower() == "kib" then return number*1024 end
	if ext:lower() == "mib" then return number*1024*1024 end
	if ext:lower() == "gib" then return number*1024*1024*1024 end
	return result
end

function raw_to_si(number)
	local function cut_number(number)
		number = tonumber(number)
	--	print ("-------")
		local s = string.format("%.5f", number)
	--	print(s)
		local dot = string.find(s, ".",0, true)
	--	print(dot)
		local whole = s:sub(0,dot-1)
		local fract = s:sub(dot+1)
	--	print(whole)
	--	print(fract)
		fract = fract:sub(0,(3-whole:len()))
		if(fract:len() == 0) then return string.format("%s", whole) end
		return string.format("%s.%s", whole, fract)
	end
	if number > (1024*1024*1024) then
		number = number/(1024*1024*1024)
		return cut_number(number).."GiB"
	end

	if number > (1024*1024) then
		number = number/(1024*1024)
		return cut_number(number).."MiB"
	end
	if number > (1024) then
		number = number/(1024)
		return cut_number(number).."KiB"
	end
	return tonumber(number).."B"
end
